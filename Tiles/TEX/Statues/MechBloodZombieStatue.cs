...
{
	public class MechBloodZombieStatue : ModTile
	{
		public int TEXCost = 6;
		public override void SetDefaults() {
			...
			name.SetDefault("Mechanical Blood Zombie Statue");
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(i * 16, j * 16, 32, 48, ItemType<MechBloodZombieStatueItem>());
		}
		...
		public override void HitWire(int i, int j)
		{
			if (GetInstance<FactoryModWorld>().worldTEX >= TEXCost)
			{
				...
				{
					npcIndex = NPC.NewNPC(spawnX, spawnY - 12, NPCID.BloodZombie);
				}
				if (npcIndex >= 0)
				{
	...
	public class MechBloodZombieStatueItem : ModItem
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Mechanical Blood Zombie Statue");
			Tooltip.SetDefault("Can be activated with wire.\nConsumes 6 T.E.X. per activation.\nMonster's regular loot table is not affected.\nSpawns all variants of the monster.");
		}

		public override void SetDefaults() {
			item.CloneDefaults(ItemID.ArmorStatue);
			item.createTile = TileType<MechBloodZombieStatue>();
			item.placeStyle = 0;
		}
        ...
        public override void AddRecipes()
        {
			...
			recipe.AddIngredient(ItemID.BloodZombieStatue, 1);
