...
{
	public class MechCrabStatue : ModTile
	{
		public int TEXCost = 1;
		public override void SetDefaults() {
			...
			name.SetDefault("Mechanical Crab Statue");
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(i * 16, j * 16, 32, 48, ItemType<MechCrabStatueItem>());
		}
		...
		public override void HitWire(int i, int j)
		{
			if (GetInstance<FactoryModWorld>().worldTEX >= TEXCost)
			{
				...
				{
					npcIndex = NPC.NewNPC(spawnX, spawnY - 12, NPCID.Crab);
				}
				if (npcIndex >= 0)
				{
	...
	public class MechCrabStatueItem : ModItem
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Mechanical Crab Statue");
			Tooltip.SetDefault("Can be activated with wire.\nConsumes 1 T.E.X. per activation.\nMonster's regular loot table is not affected.\nSpawns all variants of the monster.");
		}

		public override void SetDefaults() {
			item.CloneDefaults(ItemID.ArmorStatue);
			item.createTile = TileType<MechCrabStatue>();
			item.placeStyle = 0;
		}
        ...
        public override void AddRecipes()
        {
			...
			recipe.AddIngredient(ItemID.CrabStatue, 1);
			recipe.AddIngredient(ItemID.CrabBanner, 1);
