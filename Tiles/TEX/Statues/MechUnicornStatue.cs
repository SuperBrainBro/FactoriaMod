...
{
	public class MechUnicornStatue : ModTile
	{
		public int TEXCost = 8;
		public override void SetDefaults() {
			...
			name.SetDefault("Mechanical Unicorn Statue");
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(i * 16, j * 16, 32, 48, ItemType<MechUnicornStatueItem>());
		}
		...
		public override void HitWire(int i, int j)
		{
			if (GetInstance<FactoryModWorld>().worldTEX >= TEXCost)
			{
				...
				{
					npcIndex = NPC.NewNPC(spawnX, spawnY - 12, NPCID.Unicorn);
				}
				if (npcIndex >= 0)
				{
	...
	public class MechUnicornStatueItem : ModItem
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Mechanical Unicorn Statue");
			Tooltip.SetDefault("Can be activated with wire.\nConsumes 8 T.E.X. per activation.\nMonster's regular loot table is not affected.\nSpawns all variants of the monster.");
		}

		public override void SetDefaults() {
			item.CloneDefaults(ItemID.ArmorStatue);
			item.createTile = TileType<MechUnicornStatue>();
			item.placeStyle = 0;
		}
        ...
        public override void AddRecipes()
        {
			...
			recipe.AddIngredient(ItemID.UnicornStatue, 1);
