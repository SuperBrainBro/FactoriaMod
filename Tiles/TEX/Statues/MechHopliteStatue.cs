...
{
	public class MechHopliteStatue : ModTile
	{
		public int TEXCost = 7;
		public override void SetDefaults() {
			...
			name.SetDefault("Mechanical Hoplite Statue");
		...
		public override void KillMultiTile(int i, int j, int frameX, int frameY) {
			Item.NewItem(i * 16, j * 16, 32, 48, ItemType<MechHopliteStatueItem>());
		}
		...
		public override void HitWire(int i, int j)
		{
			if (GetInstance<FactoryModWorld>().worldTEX >= TEXCost)
			{
				...
				{
					npcIndex = NPC.NewNPC(spawnX, spawnY - 12, NPCID.GreekSkeleton);
				}
				if (npcIndex >= 0)
				{
	...
	public class MechHopliteStatueItem : ModItem
	{
		public override void SetStaticDefaults() {
			DisplayName.SetDefault("Mechanical Hoplite Statue");
			Tooltip.SetDefault("Can be activated with wire.\nConsumes 7 T.E.X. per activation.\nMonster's regular loot table is not affected.\nSpawns all variants of the monster.");
		}

		public override void SetDefaults() {
			item.CloneDefaults(ItemID.ArmorStatue);
			item.createTile = TileType<MechHopliteStatue>();
			item.placeStyle = 0;
		}
        ...
        public override void AddRecipes()
        {
			...
			recipe.AddIngredient(ItemID.HopliteStatue, 1);
